...
namespace Features
{
	...
	extern Object Create_Obj1(char* ObjectName, Vector3 Coordinates);
	extern Vector3 CombineVector(float x, float y, float z);
	extern bool stopnotifycar;
	extern bool stoptimer;
	extern int getnotify();
	extern void notifyMap(char* fmt, ...);
	extern void notifyMap(std::string str);
	...
	extern int open22;
	extern int open25;
	extern bool walkinsky;
	extern bool opened_before2;
	...
	extern void checkwhatobject();

}
namespace helpers
{
	extern bool is_key_pressed(int vKey);
	extern bool get_key_state(DWORD key, int flag);
	extern std::string get_clipboard_text();
	extern void request_control(int input);
	extern void request_control_of_id(int netid);
	extern void request_control_of_ent(int entity);
	extern std::string get_keyboard_input(const char* windowName, int maxInput, const char* defaultText);
}