...
namespace GlobalMenuVars {
	...
	void OTR(bool toggle) {
		if (NETWORK::NetworkIsSessionActive()) {
			if (toggle) {
				//Global(2657589).At(1 + (PlayerId * 466)).At(210).As<int>() = 1;
				//Global(2672505).At(57).As<int>() = NETWORK::GetNetworkTime() + 99;//56
				//Global(2657704).At(1 + (PlayerId * 463)).At(210).As<int>() = 1;
				//Global(2672524).At(57).As<int>() = NETWORK::GetNetworkTime() + 99;//56
				*ScriptGlobal::ScriptGlobal(2657921).Add(1 + (PlayerId2 * 463)).Add(210).As<int*>() = 1;
				*ScriptGlobal::ScriptGlobal(2672741).Add(58).As<int*>() = NETWORK::GetNetworkTime() + 999;//GC_OTR_TMR
	...
	void BlindEye(bool toggle) {
		if (NETWORK::NetworkIsSessionActive()) {
			if (toggle) {
				globalHandle(2793046).At(4654).As<int>() = 5;//GB_BCUT_TICK0
				globalHandle(2793046).At(4655).As<int>() = 1;//GB_PIM_BRIBE
				globalHandle(2793046).At(4657).As<int>() = NETWORK::GetNetworkTime() + 1;//GB_BCUT_TICK0
				...
				Global(2793046).At(4654).As<int>() = 0;
			}
		}
	}
	...
	void Testosterone() {
		...
		globalHandle(2672741).At(3694).As<int>() = 1; //Global_2441237.f_4013	//BULL_SHARK_TESTOSTERONE_01
	}
