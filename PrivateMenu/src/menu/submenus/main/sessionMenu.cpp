...
//		.addTooltip("This will send a pre-baked message to all players in session").addTranslation());
	...
	addOption(ToggleOption("Show as Thunder")
		.addRequirement([] { return !GetAuth()->IsVIP(); })
		.addToggle(vars.showAsThunder)
		.addHotkey().canBeSaved().addTranslation()
		.addTooltip("Show Thunder users that you're using Thunder (can take upto 1 minute to appear for others)"));

	addOption(ToggleOption("Show as Thunder VIP")
		.addToggle(vars.showAsThunderVIP)
		.addHotkey().canBeSaved().addTranslation()
		.addTooltip("Show other Thunder users that you're VIP (can take upto 1 minute to appear for others)")
		.addRequirement([] { return GetAuth()->IsVIP(); }));

	addOption(ToggleOption("~o~[Staff]~s~Show as Thunder staff")
		.addToggle(vars.showAsThunderStaff)
		.addHotkey().canBeSaved().addTranslation()
		.addTooltip("Show other Thunder users that you're staff")
		.addRequirement([] { return GetAuth()->IsStaff(); }));

	addOption(ToggleOption("~o~[Staff]~s~Bypass Thunder block")
		.addToggle(vars.bypassevoblock)
		.addHotkey().canBeSaved().addTranslation()
		.addTooltip("Allows staff to bypass Thunder block on players")
		.addRequirement([] { return GetAuth()->IsStaff(); }));
}
...
void SessionMenu::FeatureUpdate() {
	...
	if (vars.showAsThunderStaff) {
		//vars.showAsThunder = false;
		//vars.showAsThunderVIP = false;
