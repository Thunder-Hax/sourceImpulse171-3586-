...
namespace VehicleMenuVars {
	...
	void CloneVehicle(SPlayer player) {
		...
		if (ENTITY::DoesEntityExist(pedVeh)) {
			...
			GetEntityControl()->RequestModel(vehicleModelHash, [=](int model) {
				...
				auto const playerVeh = Hooked::create_vehicle(vehicleModelHash, &GetLocalPlayer().m_coordinates, GetLocalPlayer().m_heading, true, true);
				BypassPatch::ModelSpawnBypass(false)*/;
				auto const playerVeh = mCreateVehicle(vehicleModelHash, GetLocalPlayer().m_coordinates, GetLocalPlayer().m_heading, true, true);
				mVehicle_Bypass();
				...
				SpawnVehicleMenuVars::storevehicle(playerVeh, GetLocalPlayer().m_id);
	...
	void FIBNumberPlate() {
		...
		static char* message = "Thunder IS DE WAE";
