...
namespace LocalMenuVars {
	...
	/*void Regenerate() {
		static int TIMER;
		CPedb* ped = ReClass::GetWorld()->LocalPed;
		Utils::DoTimedFunction(&TIMER, 500, [&] {
			if (ped->Health < ped->MaxHealth)ped->Health += ped->MaxHealth / 100; });
	}*/
	void Regenerate() {
		static int TIMER;
		CPedb* pedbPtr = reinterpret_cast<CPedb*>(ReClass::GetWorld()->LocalPed1); // Conversion de CPed* en CPedb*
		Utils::DoTimedFunction(&TIMER, 500, [&] {
			if (pedbPtr->Health1 < pedbPtr->MaxHealth1)pedbPtr->Health1 += pedbPtr->MaxHealth1 / 100; });
		//if (ped->Health < ped->MaxHealth)ped->Health += ped->MaxHealth / 100; });
	}
	/*void Regenerate() {
	float Health = Memory2::get_value<float>({ 0x8, 0x280 });
	float MaxHealth = Memory2::get_value<float>({ 0x8, 0x2A0 });
	float healthPercent = Health * 100 / MaxHealth;
	static int TIMER;
	Utils::DoTimedFunction(&TIMER, 5, [&] {
		if (healthPercent < 90)
		{
			int myHealth = Health + 10;
			Memory2::set_value<float>({ OFFSET_PLAYER, 0x280 }, myHealth);
		}
		});
}*/
	...
	void NeverWanted(bool toggle) {
		...
		if (toggle) Memory2::set_value<int>({ OFFSET_PLAYER, OFFSET_PLAYER_INFO, OFFSET_PLAYER_INFO_WANTED }, 0);
	...
	void ChangeWanted(int level) {
		Memory2::set_value<int>({ OFFSET_PLAYER, OFFSET_PLAYER_INFO, OFFSET_PLAYER_INFO_WANTED }, playerWantedLevel);
	}
	int GetWanted(Player player) {
		return PLAYER::GetPlayerWantedLevel(player);
	}
...
void LocalMenu::Init() {
	...
	findmyid();
	playerWantedLevel = GetWanted(thatsme);
	addOption(NumberOption<int>(SCROLL, "player Wanted Level")
		.addNumber(playerWantedLevel, "%d", 1).addMin(0).addMax(5).setScrollSpeed(1).addTranslation().addHotkey()
		.addFunction([&] {ChangeWanted(playerWantedLevel); })
		.addTooltip("player Wanted Level"));
	...
	/*addOption(ButtonOption("Suicide")
		...
		//addOption(ButtonOption("Suicide")
	//	.addFunction([] {
	//		Memory2::set_value<int>({ OFFSET_PLAYER, 0x280 }, 0);  })
	//	/*.addFunction([] {ReClass::GetWorld()->LocalPed->Health = 0; })*/
	//			.addHotkey().addTranslation()
	//			.addTooltip("Suicide"));
