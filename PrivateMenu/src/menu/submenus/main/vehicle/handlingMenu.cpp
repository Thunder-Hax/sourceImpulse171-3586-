...
namespace HandlingMenuVars {
	...
	CHandlingData* GetData() {
		...
		CHandlingData* data = ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_handlingData;
...
void HandlingMenu::Init() {
	...
	addOption(NumberOption<float>(SCROLL, "Advanced suspension")
		...
		.addFunction([&] { ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_visualSuspension = -vars.suspensionVal; })
		.addOnUpdate([&] {vars.suspensionVal = -ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_visualSuspension; })
	...
	addOption(NumberOption<int>(SCROLL, "Wheel size")
		...
		.addFunction([&] { ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender->TireSize = (BYTE)vars.wheelHeightVal; })
		.addOnUpdate([] {if (VEHICLE::GetVehicleModVariation(GetLocalPlayer().m_vehicle, 23)) { vars.wheelHeightVal = (int)ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender->TireSize; }})
	...
	addOption(NumberOption<float>(SCROLL, "Wheel width")
		.addRequirement([] { return GetLocalPlayer().m_isInVehicle && ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender && GetAuth()->IsVIP(); })
		.addNumber(vars.wheelWidthVal, "%.2f", 0.1f).addMin(-1000).addMax(1000).setScrollSpeed(10)
		.addFunction([&] { ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender->m_tireWidth = vars.wheelWidthVal; })
		.addOnUpdate([] { if (ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender) vars.wheelWidthVal = ReClass::GetWorldd()->LocalPed2->m_playerVehicle2->m_vehicleDrawHandler->m_vehicleStreamRender->m_tireWidth; })
		.addTooltip("Set the wheel width").addTranslation());
