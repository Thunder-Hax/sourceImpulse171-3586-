...
namespace GriefingMenuVars {
	...
	void RamWithVehicle(SPlayer& player) {
		...
		Vehicle vehicle = mCreateVehicle(hash, coords, 1, 0, 1);
	...
	void JetAttack(SPlayer& player) {
		...
		Vehicle	veh = mCreateVehicle(Utils::GetHashKey("lazer"), pos, player.m_heading, false, 1);
	...
	void ExplosiveVehicle(SPlayer& player) {
		...
		Vehicle	veh = mCreateVehicle(Utils::GetHashKey("adder"), pos, player.m_heading, false, 1);
		if (veh != 0) {
			Ped Driver = PED::CreateRandomPedAsDriver(veh, false);
			PED::SetPedIntoVehicle(Driver, veh, -1);
