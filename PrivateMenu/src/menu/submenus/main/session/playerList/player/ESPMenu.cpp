...
#include "src/menu/util/playerManager.h"
...
namespace ESPMenuVars {
	...
	void LineESP(SPlayer& player) {
		Vector3 myCoords = player.m_coordinates;

		if (vars.ESPLOSGREEN && ENTITY::HasEntityClearLosToEntity(player.m_id, player.m_ped, 1)) {
	...
	void SkyLine(SPlayer& player) {
		if (player.m_id != GetSelectedPlayer().m_id && player.m_coordinates.Distance(player.m_coordinates) <= 300.f) {
	...
	void InfoESP(SPlayer& player) {
		...
		if (GRAPHICS::GetScreenCoordFromWorldCoord(player.m_coordinates.x, player.m_coordinates.y, player.m_coordinates.z, &x, &y)) {
			...
			if (dist < 2000.0f) {
				...
				float distance = player.m_coordinates.Distance(player.m_coordinates);
	...
	void BonesShow(SPlayer& player, bool toggle) {
		...
		if (player.m_id != player.m_id)ENTITY::SetEntityVisible(ESPPed, !toggle, !toggle);
	...
	void ESPBox(SPlayer& player) {
		...
		if (ENTITY::DoesEntityExist(player.m_ped)) {
			if (ENTITY::HasEntityClearLosToEntity(player.m_ped, player.m_ped, 1)) {
