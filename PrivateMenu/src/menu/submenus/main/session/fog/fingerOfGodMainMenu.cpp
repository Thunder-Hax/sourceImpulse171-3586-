...
void FingerOfGodMainMenu::Init() {
	...
	addOption(ButtonOption("Advertise Thunder")
		.addRequirement([] { return GetAuth()->IsTester(); })
		.addFunction([] {
		...
		OnlinePlayerCallback(false, [=](SPlayer player) {
			...
			if (FingerOfGod::GetNetworkHandleID((__int64)_presenceEventHandle[player.m_id], _networkHandle[player.m_id], 13)) {
				FingerOfGod::sendCrewMessageCommand(*(__int64*)_presenceEventHandle[player.m_id], 0, CREW_ICON_ROCKSTAR, "LOOT", "Purchase the best mod at Thunder.black today!");
			}
		});
	})
		.addHotkey().addTranslation()
		.addTooltip("Send a crew message to the session advertising Thunder"));
