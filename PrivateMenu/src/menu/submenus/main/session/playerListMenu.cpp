...
#include "src/UI/components/playerInfoComponent.h"
...
namespace PlayerListMenuVars {
	...
	String GenerateName(PlayerListMenu* instance, SPlayer& player) {
		...
		if (player.m_isThunderUserBlock) strcat(name, va("~o~%s~s~", instance->getString("[IMP-B]")));
...
void PlayerListMenu::Init() {
	...
	////playerid();
	//addOption(ButtonOption("Get The Host")
	//	.addFunction([] {
	//		SessionMenuVars::getThehost(GetSelectedPlayer().m_id);
	//		})
	//	.addTranslation().addHotkey()
	//	.addTooltip("Get The Host").addTranslation());
...
//#define SCORE_BOARD_HEADSHOT_GLOBAL 1845281 + 2 //1666485
//#define SCORE_BOARD_HEADSHOT_GLOBAL 1686330 + 2 //1666485
#define SCORE_BOARD_HEADSHOT_GLOBAL 1686471 + 2 //1686523
					...
					{
						__int64* base = Global(HeadIndex + i).Get<__int64>();
						int playerId = (int)*base;
						/*int playerId = *base;*/
						if (playerId == player.m_id)
						{
							__int64* logo = Global(HeadIndex + i + 1).Get<__int64>();
							int logos = (int)*logo;
							GPic = (char*)PED::GetPedheadshotTxdString(logos);
							GetRenderer()->DrawSpriteUnsafe({ GPic, GPic }, GetRenderer()->m_posX - (GetRenderer()->m_width / 2) + 0.01f, GetRenderer()->GetOptionY(index) + GetRenderer()->m_optionHeight / 2, 0.01125f, 0.0225f, 0, Color(255, 255, 255, 210));
							return 0.01125f;
							break;
						}
						if (playerId == -1)
