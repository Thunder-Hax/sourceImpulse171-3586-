...
namespace ExcludesMenuVars {
	...
	bool IsExcluded(SPlayer& p) {
		return (vars.host && p.m_isHost)
			...
			|| (vars.Thunder && (p.m_isEvolveUser || p.m_isThunderUserBlock))
...
void ExcludesMenu::Init() {
	...
	addOption(ToggleOption("Exclude Thunder users")
		.addToggle(vars.Thunder)
		.addTranslation().canBeSaved().addHotkey()
		.addTooltip("Exclude Thunder users"));
}
