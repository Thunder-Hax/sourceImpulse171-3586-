...
namespace DeveloperMenuVars {
	...
	Vehicle SpawnNewVehicle(char* toSpawn) {
		...
		if (STREAMING::IsModelInCdimage(model) && STREAMING::IsModelAVehicle(model)) {
			...
			/*vehicle = Hooked::create_vehicle(model, &GetLocalPlayer().m_coordinates, playerHeading, TRUE, TRUE);*/
			vehicle = mCreateVehicle(model, GetLocalPlayer().m_coordinates, playerHeading, TRUE, TRUE);



			//DECORATOR::DecorSetInt(vehicle, "MPBitset", 0);
			SpawnVehicleMenuVars::storevehicle(vehicle, GetLocalPlayer().m_id);
...
void DeveloperMenu::Init() {
	...
	addOption(ButtonOption("Print my CPed")
		.addFunction([] { LOG_PRINT("CPed: 0x%llx", ReClass::GetWorld()->LocalPed1); })
		.addTooltip("Prints my cped struct address"));

	addOption(ButtonOption("Print my CPlayerInfo1")
		.addFunction([] { LOG_PRINT("CPlayerInfo: 0x%llx", ReClass::GetWorld()->LocalPed1->PlayerInfo1); })
		.addTooltip("Prints my cplayerinfo struct address"));

	addOption(ButtonOption("Print my CPlayerInfo2")
		.addFunction([] { LOG_PRINT("CPlayerInfo: 0x%llx", ReClass::GetWorldd()->LocalPed2->PlayerInfo2); })
		.addTooltip("Prints my cplayerinfo struct address"));
