	...
	void gravshootgun() {
		...
		if (PED::GetPedConfigFlag(GetLocalPlayer().m_ped, PED_FLAG_IS_AIMING, 1)) {
			if (!ENTITY::DoesEntityExist(vehicle)) {
				...
				Vector3 Coords;
				Coords.x = GetLocalPlayer().m_coordinates.x + 5.f;
				Coords.y = GetLocalPlayer().m_coordinates.y + 5.f;
				Coords.z = GetLocalPlayer().m_coordinates.z + 7.0f;
				vehicle = mCreateVehicle(model, Coords, ENTITY::GetEntityHeading(GetLocalPlayer().m_ped), true, true);
				...
				switch (vars.shootEntityType) {
					case 0: {
						...
						Zero.z = 0;
							...
							GetEntityControl()->SimpleRequestModel(vehiclePairs[vars.vehicleVal].m_result);
							handle = mCreateVehicle(vehiclePairs[vars.vehicleVal].m_result, Zero, 0, 0, 1);
