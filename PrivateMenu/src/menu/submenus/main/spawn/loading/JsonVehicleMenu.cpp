		...
		try {
			...
			if (STREAMING::IsModelInCdimage(model) && STREAMING::IsModelAVehicle(model)) {
				GetEntityControl()->RequestModel(model, [jsonOb](int model) {
					...
					if (vars.spawnin)
					{
						playerVeh = mCreateVehicle(model, GetLocalPlayer().m_coordinates, playerHeading, true, true);
						//playerVeh = VEHICLE::CreateVehicle(model, GetLocalPlayer().m_coordinates.x, GetLocalPlayer().m_coordinates.y, GetLocalPlayer().m_coordinates.z, playerHeading, true, true, true);
					}
					else
					{
						Vector3 GetCoord;
						GetCoord.x = GetLocalPlayer().m_coordinates.x + xVect;
						GetCoord.y = GetLocalPlayer().m_coordinates.y + yVect;
						GetCoord.z = GetLocalPlayer().m_coordinates.z;
						playerVeh = mCreateVehicle(model, GetCoord, playerHeading, true, true);
						//playerVeh = VEHICLE::CreateVehicle(model, GetLocalPlayer().m_coordinates.x + xVect, GetLocalPlayer().m_coordinates.y + yVect, GetLocalPlayer().m_coordinates.z, playerHeading, true, true, true);
					}
					...
					if (vars.spawnin)
					{
						VEHICLE::SetVehicleForwardSpeed(playerVeh, vars.spawnspeed);
					}
					mVehicle_Bypass();
...
void JsonVehicleMenu::Init() {
	...
	m_addVehiclesButton = addOption(ButtonOption("~c~Add vehicles to ./Documents/Thunder/Vehicles")
		.addTooltip("Add vehicles to ./Documents/Thunder/Vehicles").addTranslation());
}
