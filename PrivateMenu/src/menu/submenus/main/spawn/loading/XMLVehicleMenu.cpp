	...
	SpoonerAttachments SpawnEntityFromXmlNode(xml_node node) {
		...
		switch (data.Type) {
			...
			case 2: { // vehicle
					...
					data.Handle = mCreateVehicle(data.ModelHash, data.Placement.Position, 1, data.Dynamic, 1);
					if (ENTITY::DoesEntityExist(data.Handle)) {
						ENTITY::FreezeEntityPosition(data.Handle, data.FrozenPosition);
...
#pragma region Root
		...
		if (root) {
			...
			Vector3 Coordinate;
			Coordinate.x = GetLocalPlayer().m_coordinates.x + xVect;
			Coordinate.y = GetLocalPlayer().m_coordinates.y + yVect;
			Coordinate.z = GetLocalPlayer().m_coordinates.z;
			...
			spoonerVehicleHandle = mCreateVehicle(vehicleHash, Coordinate, playerHeading, true, true);
