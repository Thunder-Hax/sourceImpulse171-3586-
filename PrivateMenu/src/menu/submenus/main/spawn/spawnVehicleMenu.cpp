...
namespace SpawnVehicleMenuVars {
	...
	Vehicle SpawnVehicle(Hash model, bool spawnIn, bool spawnInvincible, bool spawnMaxed, bool deleteOld, bool spawnInAir, Color primary, Color secondary, bool blip, int spawnspeed) {
		...
		if (STREAMING::IsModelInCdimage(model) && STREAMING::IsModelAVehicle(model)) {
			...
			Vector3 ourCoords = ENTITY::GetEntityCoords(PLAYER::PlayerPedId(), false);
			ourCoords.x += xVect;
			ourCoords.y += yVect;


			/*GetLocalPlayer().m_coordinates.x + xVect;
			GetLocalPlayer().m_coordinates.y + yVect;*/

			/*Vehicle veh = CHooking::create_vehicle(model, &ourCoords, heading, true, true);
			RequestControlOfEnt(veh);*/

			/*if (spawnIn) playerVeh = VEHICLE::CreateVehicle(model, ourCoords.x, ourCoords.y, ourCoords.z, playerHeading, true, true, false);
			else playerVeh = VEHICLE::CreateVehicle(model, ourCoords.x, ourCoords.y, ourCoords.z, playerHeading, true, true, false);*/
			if (spawnIn)
			{
				/*BypassPatch::ModelSpawnBypass(true);
				playerVeh = Hooked::create_vehicle(model, &ourCoords, playerHeading, true, true);
				BypassPatch::ModelSpawnBypass(false);*/
				playerVeh = mCreateVehicle(model, ourCoords, playerHeading, true, true);
			}
			else
			{
				playerVeh = mCreateVehicle(model, ourCoords, playerHeading, true, true);
			...
			else playerVeh = VEHICLE::CreateVehicle(model, GetLocalPlayer().m_coordinates.x, GetLocalPlayer().m_coordinates.y, GetLocalPlayer().m_coordinates.z, playerHeading, true, true, false);*/
			//SpawnVehicleSettingsMenuVars::Vars vars;
			//if (vars.sp_vehicle_bypass)
			//{
			//	//Global(VehicleBypass).As<BOOL>() = vars.sp_vehicle_bypass;
			...
			//}
			//else if (!vars.sp_vehicle_bypass)
			//{
			//	//globalHandle(/*VehicleBypass*/VehicleBypassGlobal[0]).As<BOOL>() = vars.sp_vehicle_bypass;
			//	*ScriptGlobal::ScriptGlobal(VehicleBypassGlobal[0]).As<int*>() = vars.sp_vehicle_bypass;
			...
			//}
			mVehicle_Bypass();
			auto networkId = NETWORK::VehToNet(playerVeh);
			ENTITY::_SetEntitySomething(playerVeh, true);
			if (NETWORK::NetworkGetEntityIsNetworked(playerVeh))
			{
				NETWORK::SetNetworkIdExistsOnAllMachines(networkId, true);
			}
			...
			if (ENTITY::DoesEntityExist(playerVeh)) {
				...
				SpawnVehicleMenuVars::storevehicle(playerVeh, isme_V);
				VEHICLE::SetVehicleIsStolen(playerVeh, false);
				if (spawnIn)
				{
					PED::SetPedIntoVehicle(GetLocalPlayer().m_ped, playerVeh, -1);
				}
    ...
    Vehicle SpawnVehiclePlayer(Hash model, int id, bool godmode) {
        ...
        if (STREAMING::IsModelInCdimage(model) && STREAMING::IsModelAVehicle(model)) {
			...
			/*BypassPatch::ModelSpawnBypass(true);
			auto playerVeh = Hooked::create_vehicle(model, &player.m_coordinates, 1, true, true);
			BypassPatch::ModelSpawnBypass(false);*/
			auto playerVeh = mCreateVehicle(model, player.m_coordinates, 1, true, true);

			//DECORATOR::DecorSetInt(playerVeh, "MPBitset", 0);
			SpawnVehicleMenuVars::storevehicle(playerVeh, id);
			...
			//{
			//	*ScriptGlobal::ScriptGlobal(VehicleBypassGlobal[0]).As<int*>() = vars.sp_vehicle_bypass;
			//	//Global(/*VehicleBypass*/VehicleBypassGlobal[0]).As<BOOL>() = vars.sp_vehicle_bypass;
			//}
			//else if (!vars.sp_vehicle_bypass)
			//{
			//	*ScriptGlobal::ScriptGlobal(VehicleBypassGlobal[0]).As<int*>() = vars.sp_vehicle_bypass;
...
Vehicle SpawnVehicleMenuVars::storevehicle(int veh, int player)
{
	DECORATOR::DecorSetInt(veh, "MPBitset", 0);
	//Entity veh = HASH::GET_HASH_KEY(vehiclestore);
	/*DECORATOR::DecorSetInt(veh, "MPBitset", 8);
	Hash netHash = NETWORK::NetworkHashFromPlayerHandle(player);
	DECORATOR::DecorSetInt(veh, "Previous_Owner", netHash);
	DECORATOR::DecorSetInt(veh, "Veh_Modded_By_Player", netHash);
	DECORATOR::DecorSetInt(veh, "Not_Allow_As_Saved_Veh", 0);
	DECORATOR::DecorSetInt(veh, "Player_Vehicle", netHash);*/
	SpawnVehicleMenuVars::RequestControlOfEnt(veh);
...
void SpawnVehicleMenu::Init() {
	...
	addSpawner(spawn++, "Money Front Hash", MoneyFrontsHashes, NUMOF(MoneyFrontsHashes));
	addSpawner(spawn++, "Bottom Dollar Bounties", BDB, NUMOF(BDB));
	addSpawner(spawn++, "Agent Of Sabotage", bodobo, NUMOF(bodobo));
	addSpawner(spawn++, "Chop Shop", chopshop, NUMOF(chopshop));
	addSpawner(spawn++, "SAMercenaries", samercenaries, NUMOF(samercenaries));
	addSpawner(spawn++, "DrugWars", DrugWars, NUMOF(DrugWars));
	addSpawner(spawn++, "Criminal", Criminal, NUMOF(Criminal));
	addSpawner(spawn++, "Contrat", contrat, NUMOF(contrat));
