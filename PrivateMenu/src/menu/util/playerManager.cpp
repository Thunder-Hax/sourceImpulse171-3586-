...
#include <Windows.h>
#include <vector>
#include "src/menu/submenus/main/session/playerList/player/ESPMenu.h"
#include "src/menu/submenus/main/session/allPlayers/sessionESP.h"
...
//	return false;
//}
bool IsSessionHost(int a1)
{
	/*if (ENTITY::DOES_ENTITY_EXIST(Hooking::get_player_ped(a1)))
	{*/
	auto netplayer = g_GameVariables->m_GetNetPlayer(a1);
	if (netplayer != nullptr && netplayer->is_host())
		return true;
	/*}
	else
		m_features::IsHostbool[a1] = 0;*/
	return false;
}
...
{
	if (SCRIPT::_GetNumberOfInstancesOfScriptWithNameHash(GET_HASH_KEY("maintransition")) != 0 || STREAMING::IsPlayerSwitchInProgress() || !g_GameVariables->m_IsSessionStarted)
		return false;
	return true;
}
...
void PlayerManager::Update() {
	...
	for (int i = 0; i < MAXPLAYERS; i++) {
		...
		player.m_name = Hooked::get_player_name(i);
		...
		int intValue = reinterpret_cast<int>(&player);
		player.m_isHost = IsSessionHost(intValue);
		...
		*/
		...
		Vector2 mvec = wtos::WorldToScreen(player.m_coordinates);

		player.m_w2s.x = mvec.x;
		player.m_w2s.y = mvec.y;
		//g_GameVariables->m_get_screen_coords_for_world_coords(player.m_coordinates, &player.m_w2s.x, &player.m_w2s.y);
		/*if (g_GameVariables->m_get_screen_coords_for_world_coords(player.m_coordinates, &player.m_w2s.x, &player.m_w2s.y))
		{
		}*/
		/*Hooked::world_screen(&player.m_coordinates, player.m_w2s.x, player.m_w2s.y);*/
	}

	m_localPlayer = &m_players[PlayerId2];
