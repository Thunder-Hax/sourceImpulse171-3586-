...
void Inventory::InvokeTransaction(const char* endpoint, std::vector<InventoryItem> items, std::function<void(bool)> callback, std::function<nlohmann::json(nlohmann::json)> modifier) {
	...
	GetThreadPool()->DoJob([=] {
		Networkk networkk(va("https://%s/gta5/11/GamePlayServices/GameTransactions.asmx/%s", GetServerUrlAndRsToken().first, endpoint));
		...
		const auto response = networkk.AdvancedPost(modified, { GetAuthHeader() });
