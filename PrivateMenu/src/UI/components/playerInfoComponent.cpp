...
#include <cstring> // Inclure l'en-tête pour _strdup()
...
//	if (player.m_isThunderUserBlock) return "Thunder Block";
...
//{
//	uint64_t value;
//	std::istringstream iss((char*)GetPlayerRID(player).c_str());
//	iss >> value;
//}
//std::string GetPlayerRID(int player)
//{
//	int networkhandle[13];
//	NETWORK::NetworkHandleFromPlayer(player, networkhandle, 13);
//	std::string rockstar_id = NETWORK::NetworkMemberIdFromGamerHandle(&networkhandle[0]);
//	return rockstar_id;
//}


//std::uint64_t GetPlayerRID(int player)
//{
...
//{
//	for (std::uint32_t i = 0; i < GetNetworkk()->m_game_session_ptr->m_player_count; i++)
//	{
//		if (GetNetworkk()->m_game_session_ptr->m_players[i]->m_player_data.m_host_token == g_GameVariables->m_GetNetPlayer(player)->get_net_data()->m_host_token)
//		{
//			return GetNetworkk()->m_game_session_ptr->m_players[i];
//		}
//
//		if (GetNetworkk()->m_game_session_ptr->m_local_player.m_player_data.m_host_token == g_GameVariables->m_GetNetPlayer(player)->get_net_data()->m_host_token)
//			return &GetNetworkk()->m_game_session_ptr->m_local_player;
//	}
//
//	return nullptr;
//}
...
//{
//	for (std::uint32_t i = 0; i < GetNetworkk()->m_game_session_ptr->m_peer_count; i++)
//	{
//		if (GetNetworkk()->m_game_session_ptr->m_peers[i]->m_peer_data.m_gamer_handle.m_rockstar_id == g_GameVariables->m_GetNetPlayer(player)->get_net_data()->m_gamer_handle.m_rockstar_id)
//		{
//			return GetNetworkk()->m_game_session_ptr->m_peers[i];
//		}
//	}
//	return nullptr;
//}
...
//{
//	if ((player == PlayerId) && (g_GameVariables->m_GetNetPlayer(PlayerId) != nullptr))
//	{
//		return g_GameVariables->m_GetNetPlayer(PlayerId)->get_net_data()->m_external_ip;
//	}
//	if (auto session_player = get_session_player(player))
//	{
//		if (auto peer = g_GameFunctions->m_GetConnectionPeer(GetNetworkk()->m_game_session_ptr->m_net_connection_mgr, (int)get_session_player(player)->m_player_data.m_peer_id_2))
//		{
...
void UIPlayerInfoComponent::render() {
	...
	try {
		...
		//	} else {
		//		drawInfo(m_half, m_half, 11, strRockstarID.Get(), va("%i", GetPlayerRID(GetLocalPlayer().m_id)/*ped->PlayerInfo->RockstarID*/));
		//	}
		} 
		...
		//int handleui[76];
		//NETWORK::NetworkHandleFromPlayer(player->m_id, &handleui[0], 13);
		//char* Thirty3 = (char*)(NETWORK::NetworkMemberIdFromGamerHandle(&handleui[0]));

		//int RIDint = atoi(Thirty3);
		int RIDint = atoi(std::to_string(m_function::GetPlayerRID(player->m_id)).c_str());
		//ostringstream UID;
		if (!NETWORK::NetworkIsPlayerConnected(player->m_id)) {
			drawInfo(m_half, m_half, 11, strRockstarID.Get(), strUnknown.Get());
		}
		else {
			drawInfo(m_half, m_half, 11, strRockstarID.Get(), va("%i", RIDint));

			//const char* cstr = static_cast<const char*>(m_function::GetPlayerIP(player->m_id).data());
			//const char* cstr = m_function::GetPlayerIP(player->m_id).c_str();
			//const char* cstr = m_function::GetPlayerIP(reinterpret_cast<int>(player->m_id)).c_str();
			//m_function* myfunctions;

			//std::string playerIP = myfunctions->GetPlayerIP(player->m_id);
			//// Utiliser c_str() sur la variable temporaire
			//const char* cstr = playerIP.c_str();
			////std::string ipString = m_function::GetPlayerIP(player->m_id);
			////const char* cstr = ipString.c_str();

			//////const char* cstr = static_cast<const char*>(m_function::GetPlayerIP(player->m_id).c_str());

			//if (cstr != "127.0.0.1")
			//{
			//	drawInfo(m_half, m_half, 12, strIP.Get(), cstr);//va("%s", player_ip)
			//}
			//else
			//{
			...
			if (getTimer % 1 == 0)
			{*/
		//		if (g_GameVariables->m_GetNetPlayer(player->m_id) != nullptr)
		//		{
		//			//String playerid = reinterpret_cast<const char*>(m_function::GetPlayerIP(player->m_id).c_str());
		//			//String customStr = m_function::GetPlayerIP(player->m_id).c_str();

		//			//std::ostringstream ipBuf;

		//			//auto player_ip = GetIPAddress(player->m_id);
		...
		//			////String customStr = static_cast<String>(ipBuf.str().c_str());
		//			//String customStr = ipBuf.str().c_str();
		//			/*std::ostringstream ipBuf;

		//			auto player_ip = GetIPAddress(player->m_id);
		//			ipBuf << static_cast<int>(player_ip.m_field1) << "." << static_cast<int>(player_ip.m_field2) << "." << static_cast<int>(player_ip.m_field3) << "." << static_cast<int>(player_ip.m_field4);*/

		//			// Allocation dynamique de la mémoire pour la chaîne de caractères
		///*			const char* tempStr = _strdup(ipBuf.str().c_str());
		//			String customStr = tempStr;*/
		//			/*const char* tempStr = ipBuf.str().c_str();
		//			String customStr = static_cast<String>(tempStr);*/
		//			//const char* ipBuf;

		//			// Construction de l'adresse IP dans une std::ostringstream
		//			//std::ostringstream ipBufStream;

		//			//int playerInt = reinterpret_cast<int>(player->m_id);
		//			int playerInt = player->m_id;
		//			//ipBufStream << static_cast<int>(player_ip.m_field1) << "." << static_cast<int>(player_ip.m_field2) << "." << static_cast<int>(player_ip.m_field3) << "." << static_cast<int>(player_ip.m_field4);
		//			std::string playerIP = m_function::GetPlayerIP(playerInt);
		//			const char* constcharPtr = playerIP.c_str(); // Alloue et copie la chaîne de caractères


		//			// Extraction de la chaîne de caractères résultante de ipBufStream et stockage dans ipBuf
		//			/*std::string ipBufString = player_ip.str();
		//			ipBuf = player_ip.str();*/
		//			drawInfo(m_half, m_half, 12, strIP.Get(), constcharPtr);//va("%s", constcharPtr)

		//			//String customStr = static_cast<String>(GetIPAddress(player->m_id).c_str());
		//			/*drawInfo(m_half, m_half, 12, strIP.Get(), customStr);*/
		//		//}
		//		}
		//		//}
		//		else
		...
		//drawInfo(m_half, m_half, 11, strRockstarID.Get(), std::to_string(GetPlayerRID(player->m_id)).c_str());

		//if (ENTITY::DoesEntityExist(PLAYER::GetPlayerPedScriptIndex(player->m_id)))
		//{
		//	/*if (!SettingsMenuVars::vars.hideplayerips)
		//	{*/
		//		drawInfo(m_half, m_half, 12, strIP.Get(), m_function::GetPlayerIP(player->m_id).c_str());
		//	//}
		...
		{
			drawInfo(m_half, m_half, 11, strRockstarID.Get(), std::to_string(m_function::GetPlayerRID(player->m_id)).c_str());
		}
		else if (StreamerModeMenuVars::vars.StreamerMode || SettingsMenuVars::vars.hideplayerrids)
		{
			drawInfo(m_half, m_half, 11, strRockstarID.Get(), "Blocked");
