...
void WindowManager::Init() {
	...
	//#define YTD_1_SIZE 428824 //Basic Thunder
	//#define YTD_1_SIZE 1625396 //Advanced Thunder
	//#define YTD_1_SIZE 1629599 //Advanced Thunder Extras
	#define YTD_1_SIZE 2726432 //Advanced Thunder Extras + more headers
	...
	LOG_DEVDEBUG("Systemdata window");
	/*Systemdata window*/
	...
	LOG_DEVDEBUG("Vehicle info window");
	/*Vehicle info window*/
	...
	LOG_DEVDEBUG("Drawable info window");
	...
	LOG_DEVDEBUG("Player info window");
	/*Player info window*/
	...
	LOG_DEVDEBUG("registerScrollEvent");
	GetMouse()->registerScrollEvent([&](int scroll) {m_frame.scrollEvent(scroll); });

	/*MP vehicles in SP*/
	//Global(Globals::MPINSP).As<int>() = 1;
	LOG_DEVDEBUG("UpdateMouse");
	/*Init the mouse*/
	GetMouse()->UpdateMouse();
	LOG_DEVDEBUG("GetScreenPanel");
	GetScreenPanel()->Init();

	//GetMenu()->Initialize();
	//m_frame.addWindow(GetMenu(), false);
	LOG_DEVDEBUG("GetMenu");
	GetMenu()->Initialize();
	m_frame.addWindow(GetMenu(), false);
	LOG_DEVDEBUG("SetHeader");
	ThemeMenuVars::SetHeader(1);
	LOG_DEVDEBUG("LoadConfig");
	LoadThemesMenuVars::LoadConfig();

	static char message[64];
	LOG_DEVDEBUG("Welcome");
	vaBuff(message, 64, "Welcome %s ~b~- ~y~%s", GetAuth()->GetUsername(), GetAuth()->SRole());
	//GetScaleform()->DrawLargeMessage(u8"~b~∑ ~s~Thunder ~b~∑", message, 10, 600);
	GetScaleform()->DrawLargeMessage(u8"~r~∑ ~s~Thunder ~r~∑", "", 10, 600);
...
void WindowManager::Update() {
	...
	if (GetAuth()->IsAuthorized()) {
		...
		if (m_firstOpen) GetScaleform()->DrawInstructional("Open Thunder ", g_keyNamesInstructional[GetMenu()->m_openKey]);
