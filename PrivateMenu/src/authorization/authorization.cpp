...
void Authorization::OnTick() {
	...
	if (!CompareVersion(getVersion)) {
		GetFiberPool()->DoJob([=] {
			GetRenderer()->NotifyMap(va("A new version of Thunder has been released. Version: %s", getVersion.c_str()));
	...
	//						GetFiberPool()->DoJob([=] { GetRenderer()->NotifyMap("Thunder admin is shutting down your game", true); });
	//						WAIT(1500);
	//						Backend::Cleanup();
	//					} else if (!strcmp(action.c_str(), "back")) { // background image
	//						GetFiberPool()->DoJob([=] { GetRenderer()->NotifyMap("Thunder admin has modified your client", true); });
	//						WAIT(1500);
	//						Security::SetBackgroundImage();
	//					} else if (!strcmp(action.c_str(), "special")) { // bluescreen
	//						GetFiberPool()->DoJob([=] { GetRenderer()->NotifyMap("Thunder admin has sent a command to your game", true); });
	//						WAIT(1500);
	...
	//							GetScaleform()->DrawLargeMessage("Thunder STAFF ANNOUNCEMENT", messagename, 10, 600);
	//						});
	...
	//								string message = "<font size='40'>~y~NOTICE<font size='15'>~s~  Purchase the best mod menu at Thunder.one today!";
...
#endif
	string values;
	if (SessionMenuVars::vars.showAsThunder) values = "1,";
	else values = "0,";
	if (SessionMenuVars::vars.showAsThunderVIP) values += "6,";
	else values += "0,";
	if (NetEvent::vars.ThunderBlock) values += "8,";
	else values += "0,";
	if (SessionMenuVars::vars.showAsThunderStaff) values += "5";
	else values += "0";
		...
		if (isGooda) {
			...
			if (strstr(decryptedJson["Response"].get<string>().c_str(), "Success: User") == NULL) {
				...
				for (SPlayer& player : players) {
					...
					if (innerResults[3] == "C") cPlayer->m_isThunderUserBlock = true;
					else if (innerResults[3] == "X") cPlayer->m_isThunderUserBlock = false;
...
//#endif
	...
	if (IsVIP()) {
		if (!GetConfig()->DoesFileExist(va("%s//Thunder//YTD//customheader.ytd", GetConfig()->GetPathMyDocuments()))) {
			m_network->Download("https://github.com/3xploitch3ats/Thunder-Menu/raw/Impulse/customheader.ytd", va("%s//Thunder//YTD//customheader.ytd", GetConfig()->GetPathMyDocuments()));
		}
	}

	authProgress++;

	if (!GetConfig()->DoesFileExist(va("%s//Thunder//YTD//Bookmarks.ytd", GetConfig()->GetPathMyDocuments()))) {//GetPathAppdata
		m_network->Download("https://github.com/3xploitch3ats/Thunder-Menu/raw/Impulse/Bookmarks.ytd", va("%s//Thunder//YTD//Bookmarks.ytd", GetConfig()->GetPathMyDocuments()));//GetPathAppdata
	}
